...
22.1 Revision 1.0
 ...
 Approved version of LoRaWAN1.022.2 Revision 1.0.1
...
payload will be encrypted, using any method, at the application layer then re-encrypted at the MAC layer using the specified default LoRaWAN encryption
...
avoid end-device’s hidden state problem. (in 5.4 and 5.7)
 ...
 Added a frequency plan for the Australian 915-928MHz ISM band22.3 Revision 1.0.2
...
frequency at which an end-device expects a downlink.
 ...
 Added the Tx ParamSetupRec MAC command. This command enables to remotelymodify the maximum TX dwell time and the maximum radio transmit power of a 
device in certain regionsLoRaWAN 1.1 Specification©2017 LoRa Alliance™ Page 96 of 101 The authors reserve the right to change 
...
and LoRaWAN1.0.2.22.4.1 Clarificationso Grammatical o Normative text used consistently o ADR behavior, 
 ...
 ADR Backoff behavior o Default TXPower definition o FCnt shall never be reused with the same session keys o MAC Commands are discarded if present in both FOpts and Payload o Retransmission backoff clarification 22.4.2 Functional modificationso FCnt changes 
...
Unnecessary with 32bit counters 
 ...
 ABP device must never reset frame counters o Retransmission (transmission without incrementing the FCnt) 

 Downlink frames are never retransmitted 

 Subsequent receptions of a frame with the same FCnt are ignored 

 Uplink retransmissions are controlled by NbTrans (this includes both confirmed and unconfirmed frames) LoRaWAN 1.1 Specification©2017 LoRa Alliance™ Page 97 of 101 The authors reserve the right to change 
specifications without notice.
 ...
 Class A device cease retransmitting a frame upon the reception of a frame in either the RX1 or RX2 windowo Key changes 
...
Keys fully defined later (section 6) 
 ...
 Associated MIC and Encrypt changes using new keys o MAC Commands introduced 
...
Value of 0 is to be ignored for NbTrans o Activation 
...
NwkSEncKey (Encrypt up/down) LoRaWAN 1.1 Specification©2017 LoRa Alliance™ Page 98 of 101 The authors reserve the right to change 
specifications without notice.
...
JSencKey (Join-Accepts in response to Rejoin-Request) 
 ...
 Session context defined o OTAA 
...
1.0 operation reversion by a 1.1 device defined o ABP 

 Additional Session key requirement described o Class B 
 ...
 Deprecated the BeaconTimingReq/Ans (replaced by the standard MAC command DeviceTimeReq/Ans) o Class C 

 Clarify requirement for a DL timeout 

 Add Class C MAC Commands 

DeviceModeInd/Conf 22.4.3 Examples
